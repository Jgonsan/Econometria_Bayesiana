...
reg_freq <- lm(num_mortes ~ ratio_permit_totals + all_male_participants, data=guns_data)
summary(reg_freq)

reg_bayes <- brms::brm(num_mortes ~ ratio_permit_totals + all_male_participants, data=guns_data)
...
# investigate model fit= Compara os dados com a predição.
pp_check(reg_bayes)
